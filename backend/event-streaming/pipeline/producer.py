...
# Step 3: run producer.py to start sending data through Kafka
# =============================================================================
...
def updatePortfolio():
    ...
    empty={"empty":"empty"}
    producer.send(topic='portfolio_update', value=empty)
...
if __name__=="__main__":
    ...
    # symbol = "AAPL"
    # value,time_zone=get_historical_data(symbol=symbol,outputsize='compact')
    # # print("in real 1")
    # now_timezone=datetime.datetime.now(timezone(time_zone))
    # # transform ready-to-send data to bytes, record sending-time adjusted to the trading timezone
    # # print("in real2")
    ...
    schedule.every(10).seconds.do(updatePortfolio)
    ...
    if datetime.datetime.now(timezone(timeZone)).time()>datetime.time(16,0,0) or datetime.datetime.now(timezone(timeZone)).time()<datetime.time(9,30,0):
        schedule.every(10).seconds.do(kafka_producer_fake,producer,'AAPL')
    else:
        # ['AAPL', 'NFLX', 'WFC', 'GOOG', 'MSFT']
        schedule.every(60).seconds.do(kafka_producer,producer,'AAPL',True)
        schedule.every(60).seconds.do(kafka_producer,producer,'GOOGL',True)
        schedule.every(60).seconds.do(kafka_producer,producer,'NFLX',True)
        schedule.every(60).seconds.do(kafka_producer,producer,'WFC',True)
        schedule.every(60).seconds.do(kafka_producer,producer,'MSFT',True)
